(function($){
		...
		self.methods._add('methods', {
			mobile : {
				...
				_set_mobile :			function() {
											var is_mobile = self.methods.mobile._is_browser_mobile();
											if( is_mobile ) {
				...
				_is_mobile :			function(width) {
								
											if( !width ) {
												
												var body = typeof self.elements.body !== 'object' ? $('body') : self.elements.body;
				...
				_is_width :				function(size,larger_smaller) {
					
											size = size ? size : 991;
											larger_smaller = larger_smaller ? larger_smaller : 'smaller';
	...
	$window.on(window.object_name+'._document_ready', function(object_event, self) {
		...
		self.methods.mobile._set_mobile();
	});
