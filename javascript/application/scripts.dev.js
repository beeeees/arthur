...
(function($){
	...
	window[window.object_name] = {
		...
		_initialize		:	function(){
								...
								$window.trigger(window.object_name+'._initialize', self);
								$window.trigger(window.object_name+'._add_methods', self);
		...
		methods			:	{
								...
								_setup_events :		function(events) {
														for( var event in events ) {
															...
															if( events[event].delegate ) {
																events[event].delegate.on(e, events[event].element, events[event].action);
															}else if ( events[event].data ) {
	...
	window[window.object_name]._initialize();
